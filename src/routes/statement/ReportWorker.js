...
export default class ReportMachine extends Component {
            ...
            res.json().then(function (data) {
              if (data.code === "0") {
                  that.setState({
                     ...
                     workerList: JSON.parse(data.data).userList,
                     firstWorker: JSON.parse(data.data).userList[0].name,
                     selectInput: JSON.parse(data.data).userList[0].id,
                     listName: '今日',
                  });
              } else {
  ...
  allData = (type) => {
    ...
    formData.append('active', 1);
    formData.append('type', 'user');
    formData.append('userId', this.state.selectInput);
    if(type === 'yesterday') {
      formData.append('dateFormat', 'hour');
          ...
          <Card loading={loading} bordered={false} bodyStyle={{ padding: 0 }}>
            <div className={styles.salesCard}>
              <Tabs tabBarExtraContent={salesExtra} size="large" tabBarStyle={{ marginBottom: 24 }}>
                <TabPane tab="挡车工产量"  key="sales">
                  <div className={styles.totalNumber}>
                     <span className={styles.totalSpan}>总产量：{this.state.totalNumber}双</span>
                     <span style={{ float: 'right' }}>
                         挡车工姓名：
                         {getFieldDecorator('input', { initialValue: this.state.firstWorker })(
                            <Select style={{ width: '120px' }} onChange={this.onChange} placeholder='选择挡车工'>
                                {
                                   this.state.workerList.map(function (item) {
                                   return (
