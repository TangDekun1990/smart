...
export default class ReportGroup extends Component {
            ...
            res.json().then(function (data) {
              if (data.code === "0") {
                  that.setState({
                     ...
                     oneGroup: JSON.parse(data.data).groupList[0].name,
                     selectInput: JSON.parse(data.data).groupList[0].id,
                     listName: '今日',
                  });
              } else {
  ...
  allData = (type) => {
    ...
    formData.append('active', 1);
    formData.append('type', 'group');
    formData.append('workshop', this.state.selectInput);
    if(type === 'yesterday') {
      formData.append('dateFormat', 'hour');
  ...
  // 选择下拉框事件
  onChange = (value) => {
    this.setState({selectInput: value},() => {this.allData()})  // 异步操作,使用回调函数进行处理
  }
          ...
          <Card loading={loading} bordered={false} bodyStyle={{ padding: 0 }}>
            <div className={styles.salesCard}>
              <Tabs tabBarExtraContent={salesExtra} size="large" tabBarStyle={{ marginBottom: 24 }}>
                <TabPane tab="组别产量"  key="sales">
                  <div className={styles.totalNumber}>
                    <span className={styles.totalSpan}>总产量：{this.state.totalNumber}双</span>
                    <span style={{ float: 'right' }}>
                        组别名称：
                        {getFieldDecorator('input', { initialValue: this.state.oneGroup })(
                           <Select style={{ width: '120px' }} onChange={this.onChange} placeholder='选择组别'>
                               {
