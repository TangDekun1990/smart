            ...
            res.json().then(function (data) {               
                if (data.code === "0") {
                  // console.log(JSON.parse(data.data).stockList);
                    that.setState({listShow: JSON.parse(data.data).stockList});  
...
//                   that.setState({listShow: data.data.stockList});  
...
// }
 ...
 //  删除库存
  remove = (id) => {
    ...
    formData.append('version', 1);
    fetch( URL + '/stock/deleteStockById/v1', {
        method: 'post',
        mode: 'cors',
        body: formData
    }).then(function(res) {
  ...
  doUpdate = (id) => {
    ...
    const { dispatch } = this.props;
    dispatch(routerRedux.push('/stocks/update'));
  };
  //排序
  setAgeSort = () => {
    this.setState({
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        sortOrder: sortedInfo.columnKey === 'productQuantity' && sortedInfo.order,
      },
      {
        title: '物品单位',
        dataIndex: 'unitValue',
