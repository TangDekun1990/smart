...
export default class Warning extends PureComponent {
  ...
  componentDidMount() {
    ...
    fetch('/stockalert/updateStockAlertById/1', {
      method: 'post',
      mode: 'cors',
      body: formData
    }).then(function(res) {
      if (res.ok) {
        res.json().then(function (data) {
          if (data.code === '0') {
            that.setState({listShow: data.data.stockAlert});
          }
        });
      ...
      if (res.ok) {
        res.json().then(function (data) {
          if (data.code === '0') {
            alert(data.msg);
            setTimeout(function(){
              dispatch(routerRedux.push('/stocks/alert'));
            }, 500);
          }
        });
      } else if (res.status === 401) {
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row gutter={{ md: 8, lg: 24, xl: 48 }} style={{  marginLeft: '21%' }}>
          <Col md={8} sm={24}>
            <FormItem label="订单编号">
                <Select placeholder="请选择" value={this.state.listShow.stockNumber}>
                  <Option value="0">N2018020179</Option>
                  <Option value="1">L2018020969</Option>
                  <Option value="2">A2018020699</Option>
                  <Option value="3">201803011052</Option>
                </Select>
            </FormItem>
          </Col>
          <Col md={8} sm={24}>
            <FormItem label="预警数量">
                <Input placeholder="请输入" value={this.state.listShow.number} />
            </FormItem>
          </Col>
        </Row>
        <Row gutter={{ md: 8, lg: 24, xl: 48 }} style={{  marginLeft: '21%' }}>
