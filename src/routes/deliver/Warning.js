...
export default class Warning extends PureComponent {
  ...
  componentDidMount() {
    ...
    fetch( '/deliveritemalert/getDeliverItemAlertList/1', {
      method: 'post',
      mode: 'cors',
      // body: formData
    }).then(function(res) {
      if (res.ok) {
        res.json().then(function (data) {
          if (data.code === '0') {
            // 存用户列表和工厂列表的数据
            that.setState({listShow: data.data.deliveritemAlertList})
  ...
  renderSimpleForm() {
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        ...
        <Row md={8} sm={24} style={{  textAlign: 'center' }}>
          <span className={styles.submituttons}>
  ...
  render() {
    ...
    const columns = [
      {
        title: '发货编号',
        dataIndex: 'deliverItemNumber',
        key: 'deliverItemNumber',
      },
      {
        title: '订单编号',
    ...
    const { dispatch } = this.props;
    const addWarn = () => {
      dispatch(routerRedux.push('/deliver/newWarn'));
    };
    const updateWarn = () => {
      dispatch(routerRedux.push('/deliver/updateWarn'));
    };
