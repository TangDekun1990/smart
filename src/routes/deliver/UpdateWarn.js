...
export default class Warning extends PureComponent {
  ...
  componentDidMount() {
    ...
    fetch('/deliveritemalert/updateDeliverItemAlertById/1', {
      method: 'post',
      mode: 'cors',
      body: formData
    }).then(function(res) {
      if (res.ok) {
        res.json().then(function (data) {
          if (data.code === '0') {
            that.setState({listShow: data.data.deliveritemAlert});
          }
        });
      ...
      if (res.ok) {
        res.json().then(function (data) {
          if (data.code === '0') {
            alert(data.msg);
            setTimeout(function(){
              dispatch(routerRedux.push('/deliver/alert'));
            }, 500);
          }
        });
      } else if (res.status === 401) {
          ...
          <Col md={8} sm={24}>
            <FormItem label="订单编号">
                <Select placeholder="请选择" value={this.state.listShow.orderNumber}>
                  ...
                  <Option value="3">201803011106</Option>
                </Select>
            </FormItem>
          </Col>
          <Col md={8} sm={24}>
