...
export default class Warning extends PureComponent {
  ...
  componentDidMount() {
    ...
    fetch('/orderalert/getOrderAlertList/v1', {
        method: 'post',
        mode: 'cors',
       body: formData
    }).then(function(res) {
        if (res.ok) {
            res.json().then(function (data) {
                    if (data.code === '0') {
                        // 存用户列表和工厂列表的数据
                        that.setState({listShow: data.data.orderAlertList});
                    }
            });
  ...
  render() {
    ...
    const { dispatch } = this.props;
    const addWarn = () => {
      dispatch(routerRedux.push('/order/newWarn'));
    };
    const updateWarn = () => {
      dispatch(routerRedux.push('/order/updateWarn'));
    }
      ...
      <PageHeaderLayout>
        <Card bordered={false}>
          <div className={styles.tableList}>
            ...
            <div style={{width: '100%', textAlign: 'right', marginBottom: 20, marginRight: 10}}>
