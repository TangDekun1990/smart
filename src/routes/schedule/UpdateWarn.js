...
export default class UpdateWarn extends PureComponent {
  ...
  componentDidMount() {
    ...
    fetch('/planproductalert/updatePlanProductAlertById/1', {
      method: 'post',
      mode: 'cors',
      body: formData
    }).then(function(res) {
      if (res.ok) {
        res.json().then(function (data) {
          if (data.code === '0') {
            that.setState({listShow: data.data.planProductAlertList});
          }
        });
      ...
      if (res.ok) {
        res.json().then(function (data) {
          if (data.code === '0') {
            alert(data.msg);
            setTimeout(function(){
              dispatch(routerRedux.push('/schedule/alert'));
            }, 500);
          }
        });
      } else if (res.status === 401) {
  ...
  renderSimpleForm() {
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
          ...
          <Col md={8} sm={24}>
            <FormItem label="订单编号">
                <Select placeholder="请选择" value={this.state.listShow.orderNumber}>
                  ...
                  <Option value="3">201803011141</Option>
                </Select>
            </FormItem>
          </Col>
          <Col md={8} sm={24}>
            <FormItem label="预警节点">
                <Select placeholder="请选择" value={this.state.listShow.node}>
        ...
        <Row gutter={{ md: 8, lg: 24, xl: 48 }} style={{  marginLeft: '21%' }}>
          <Col md={8} sm={24}>
            <FormItem label="再次提醒时间间隔">
                <Input placeholder="请输入" value={this.state.listShow.timeAgain} />
            </FormItem>
          </Col>
        </Row>
        <Row gutter={{ md: 8, lg: 24, xl: 48 }} style={{  marginLeft: '21%' }}>
        ...
        <Card bordered={false}>
          <div className={styles.tableList}>
            ...
            <div style={{width: '100%', textAlign: 'center'}}>
              <Button type="primary" onClick={ this.addSave }>保存</Button>
            </div>
          </div>
        </Card>
      </PageHeaderLayout>
